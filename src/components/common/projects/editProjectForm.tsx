...
// api
import { editProjectApi } from "@/src/lib/api/projects/editProject";
...
// schemas
import { addProjectSchema as editProjectSchema } from "@/src/lib/schemas/addProjectSchema";

// types
import { AddProjectFormData as EditProjectFromData } from "@/src/lib/schemas/addProjectSchema";
...
const EditProjectForm = ({ project }: ProjectEditProps) => {
  ...
  const onSubmit: SubmitHandler<EditProjectFromData> = async (data) => {
    setIsLoading(true);
    try {
      await editProjectApi(data, project.id);
      toast.success("Project updated successfully!");
    } catch {
      toast.error(`Failed to update project: ${errors}`);
    } finally {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-16 sm:p-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h3 className="font-semibold text-2xl">Edit Project</h3>
